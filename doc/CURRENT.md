...
| SAR | `.sar` | Yes | Yes | Yes | Store/` LH0 `, ` LH4 `, ` LH5 ` | Streamline Design's SAR is LHA with a differently spelled method tag, so `XSAR` inherits the LHA container reader. A byte comparison against an LHA 2.13 archive of the same payload showed the two differ in ten bytes — the tag, the header checksum that follows from it, the OS identifier and a zeroed timestamp — while the header fields, CRC-16, the whole compressed stream and the terminator were identical, and the stream decoded to the same SHA-256 as an independent copy of the payload. Detection is stricter than LHA's because SAR carries no magic number and `.sar` is a heavily overloaded extension: a supported tag, header level 0 or 1, and a verified header checksum are all required. SAR.DOC documents exactly three methods — ` LH5 `, ` LH4 ` and ` LH0 ` — and all three are mapped; any other tag and header level 2 fail closed. (There is no proprietary `-sar-` method; that claim appeared only in an issue note and is not in the format documentation.) ` LH4 ` is byte-verified against the project's own genuine `-lh4-` archive, re-tagged with a recomputed header checksum so the compressed stream is authentic. That test asserts the **resolved handler**, not the payload: methods 4 and 5 differ only in `pos_pt_len_size` (13 vs 14) and the 0x20 window prefill (4 KiB vs 8 KiB), neither of which an lh4 encoder can exercise, so an lh4 stream decoded as lh5 still yields the correct bytes and a byte-parity test could not catch a mis-map. Only ` LH5 ` has been observed in a real archive; ` LH0 ` remains mapped on the documentation alone. |
...
| ARC (SEA) | `.arc` | Yes | Yes | Partial | Stored (1–2), Packed/RLE (3), Squeezed/Huffman (4), Crunched dynamic LZW (8), Squashed (9) | Every method except the obsolete 5/6/7 extracts. 337 entries across the 18 real ARC archives available here restore with the exact declared size and a matching CRC-16/ARC, recomputed independently of the application. Methods 5–7 are ARC's original hash-table crunch — a different decompressor, not the LZW below with a fixed width. Reading the ARC 5.21 source settles what that means concretely: `arcunp.c` dispatches 5/6/7 to `init_ucr`/`getb_ucr` while 8/9 go to `decomp()`; `upd_tab` places entries at `string_tab[hash(pred,foll)]` and `init_tab` hashes even the 256 atomic codes, so a literal's code is not its byte value; codes are nybble-packed via `GOCODE`; and there is no CLEAR. **Methods 6 and 7 do not decode identically** — `init_ucr(1,f)` selects `newh()` for method 7 only — so implementing these needs three distinct handles, not two. No surviving archive using them was found on disk (0 across ~347 members in 49 local `.arc` files) or in a 212-item sweep of the Internet Archive's DOS shareware collection, so they map to `HANDLE_METHOD_UNKNOWN` and are refused on every route rather than published as their compressed bytes. Detection uses the method's own header length — 25 bytes for method 1, which carries no original-size field, and 29 for methods 2–9 — rather than a flat 29-byte floor that rejected valid method-1 archives period tools accept. |
| FreeARC | `.arc` | Yes | No | No | — | `XFREEARC::getFileParts` emits raw blocks only; no per-file records |
| CAB | `.cab` | Yes | Yes | Yes | Store, MSZIP, LZX, Quantum | The filename route's compiled ip7z handler includes Quantum. The native `XCab` path now also decodes Quantum via a first-party MIT `XQuantumDecoder` (clean-room from Russotto's public spec, not libmspack), wired at all three type switches. **The native Quantum decoder is thinly verified**: real-Quantum conformance rests on a single 59-byte vector (`mixed.cab`'s `qtm.txt`, order 18) pinned in the `runDirectCabBlock` device harness, with the remaining code paths covered only by an internal encoder round-trip. `FT_CAB` is not native-preferred, so the default console path still uses ip7z; the native decoder is reachable only by direct `QIODevice` callers. |
...
| AutoIt | `PE32/PE64: AutoIt` | Yes | **Yes** | **Yes** | AutoIt v2 and v3/EA06 are handled. An official v2.64 compiler plus Exe2Aut oracle verifies LCG-encrypted record parsing, JB01 stored/compressed data, exact script and `FileInstall` payloads, routing, quotas, and malformed rejection. An official AutoIt 3.3.18.0 EA06 fixture independently verifies two records and an exact `FileInstall` payload. **v2 detection no longer requires the trailing backlink DWORD** to point at the container: some genuine Aut2Exe builds leave it indexing the PE image while the container sits at the overlay, so `_detect` also locates the container by its 16-byte magic and `_parseV2` validates by the record chain (every marker must decrypt to `FILE` and the walk must land exactly on the trailer) rather than the backlink. The magic is a fixed constant absent from every non-AutoIt file in the local corpora, so scanning does not false-positive; `console_autoit_v2_regression.ps1` pins a real such sample (its backlink points into `.text`) extracting its 26-byte script. |
...
| ASPack | `PE32: ASPack` | Yes | Partial | PE32 only. Stub generations 2.00, 2.001/2.1, **2.11/2.11c/2.11d/2.11r**, 2.12, 2.2 and 2.42 are handled: the production CLI reconstructs **88/128** current corpus executables, every one a valid restored PE carrying the original OEP; the pinned 2.12 fixture additionally has its whole code section byte-identical to the pre-pack input. **2.11 hides the call/jmp mark byte and the stored OEP inside a per-file polymorphic self-decryptor**, so (when built `WITH_XEMULATOR`) `XASPACK` emulates the stub head from the entry point until the deterministic post-decrypt landing, copies the decrypted head back, and then reads the mark and OEP exactly as for the plaintext generations — all 32 samples restore byte-exact against their pre-pack oracles, and any emulation fault/step-cap/cancel falls back to the generic route so a mis-emulated file is never restored with a wrong OEP. The 1.0x stubs (40 samples) use a different pre-2.0 compressor and fall back to raw PE-section records. Each generation is selected by an exact entry signature plus the constant decoder table at that generation's offsets (the 2.11 rows omit the marker check, which is inside their encrypted head, and rely on the signature + table). |
...
| Item | Kind | Notes |
|---|---|---|
| Repair runtime-failing static routes | Correctness | MEW is complete (16/16), FSG is complete (12/12), ASPack 2.11 is complete (32/32 byte-exact via the emulated stub-head decrypt). ASPack is at 88/128; what remains is only the ASPack 1.0x pre-2.0 compressor. Preserve family-specific negative checks. Tracker #12 |
...
**Destination path length.** There is no longer a total-path ceiling. Qt's own
file classes are long-path capable, but this project also makes raw Win32
`GetFileAttributesW`/`SetFileAttributesW` calls, and those were capped at the
legacy 260-character `MAX_PATH`. They fail *closed*: a perfectly ordinary
just-published file past the cap returned `INVALID_FILE_ATTRIBUTES` /
`ERROR_PATH_NOT_FOUND`, and the fail-closed fallback then reported it as an
unsafe reparse point, aborting the extraction and stranding a rollback journal.
All five such call sites now go through `XBinary::winExtendedNativePath`, which
applies the `\\?\` extended-length form (and `\\?\UNC\` for shares).

Two independent sites had to be fixed, and a single deep-destination test can
pass while the other is still broken, so both are pinned in
`console_ip7z_safety_regression.ps1`: the **staging** site, where
`parent + "\.xfileunpacker-staging-XXXXXX\" + member` overflows, and the
**publication** site, where `destination + member` overflows and is checked by
`pathIsUnsafeLink`. Measured after the fix: a 948-entry CHM and a 1,197-entry
IPA both extract completely into a 344-character destination, with no stranded
`.xunpack-rollback-*` or staging state; before it, extraction failed at a
260-character total.

The surviving limit is NTFS's **255 characters per path component**, which the
extended-length form does not lift. A single directory or file name longer than
that is rejected by the filesystem itself, not by this project.
...
On the native/static route, `archivePathHasUnsafeLink` walks each destination
path component and now rejects any component carrying
`FILE_ATTRIBUTE_REPARSE_POINT`, not only those Qt's `isSymLink()` recognizes —
the same tag-agnostic check the ip7z route already used. This closes a Windows
asymmetry where junctions/mount points and cloud/dedup/app-exec-link reparse
tags could pass the native check. `console_native_safety_regression.ps1` drives
a native `.tar.gz` into a junctioned destination component and confirms the
extraction fails closed without writing through the junction. Hardlinks, which
carry no reparse tag, remain uncovered on every route.
...
| Static packer route and breadth regression | Five canonical ASPack/FSG/MEW/NsPack/Petite CLI routes list reconstructed records; **eleven** direct packer fixtures produce valid PEs, preserve the oracle OEP, and reproduce the oracle's ENTIRE code section byte-for-byte. The original seven all shared the 2,048-byte `in_msvc.exe` oracle, so "entire code section" was only ever 512 bytes for the five analysis-dump families (15,872 / 32,256 for UPX PE32 / PE64) — and `in_msvc`'s entry point at RVA 0x1000 is exactly the first section's VA, which is also `XPETITE`'s fallback when OEP recovery fails, making that assertion degenerate. Three rows added on 2026-08-24 use larger, different-compiler oracles: ASPack/bcc (36,864-byte CODE plus a named 1,536-byte `.idata`), FSG/fpc (28,160-byte `.text`, entry point 0x1540) and MEW/delphi (512-byte `.itext` plus a named 35,328-byte `.text`, entry point 0xa0f4). The two non-degenerate entry points cannot be satisfied by a fallback. The optional named section is resolved by name in the oracle and then by RVA in the output — never by section index, since these are rebuilt section tables — and fails closed if the name is absent. The two UPX outputs additionally execute and exit 0x12345678; the other five fault with 0xC0000005 because their unpackers deliberately do not rebuild the import directory, so execution is not a reachable gate for them (XFU-031). NsPack passed 34/34 corpus listings and Petite 2/2. ASPack is **88/128**, FSG **12/12**, and MEW 16/16; every reconstructed output in those three families was extracted through the shipped console and compared against the staged pre-pack input, matching the original entry-point RVA and its first 64 bytes in all 116 cases. FSG 1.1/1.2 additionally has one encrypted-stub fixture (`1.2/in_fpc`) whose whole 28,160-byte code section is byte-identical with a non-degenerate 0x1540 OEP. ASPack 2.11/2.11c/2.11d/2.11r reconstruct byte-exact via an emulated stub-head decrypt (32 samples, code + OEP verified against the oracles). The remaining ASPack 1.0x variants fall back as documented. One sample from each of 54 PE UPX version directories still lists as UPX, with breadth byte comparison open. |
...
| Long destination paths | A 948-entry CHM (`wpperrs.chm`) and a 1,197-entry IPA (`ipad.ipa`) each extract completely into destinations of 127, 255 and 344 characters, exit 0, with zero stranded rollback or staging state. Before the `\\?\` fix, extraction failed closed at a 260-character total and left the destination dirty — one published file plus an orphaned `.xunpack-rollback-*` journal. Both the staging and publication sites are pinned as separate cases, because a deep-parent/short-leaf archive exercises only the first and a short-parent/long-leaf archive only the second. Residual: NTFS still rejects any single path component over 255 characters, which no prefix lifts. |
| UDF cross-reader oracle | The UDF and ISO 9660 readers extract byte-identical payloads from the same 1.5 MiB volume — `README.md` 30 B, `bin/ls` 38,688 B and `etc/services` 677,972 B, all three SHA-256-matched — with the UDF route reached only via a run-time-derived pure-UDF image. Two independent readers agreeing on one volume is a stronger assertion than either matching a hash of itself. Repeated sessions on the same archive are also pinned, because `XUDF` previously had no `ownsUnpackSource` gate ahead of its context delete. Source immutability is checked before and after. |
| Device-cursor contract | `isValid()` and `getFileFormatInfo()` restore the caller's device position across `XLHA`, `XSAR`, `XARX`, `XSEAARC`, `XNPM`, `XAPK`, `XIPA`, `XJAR`, `XAPKS` and both `XDEB` entry points. This is a real bug class, not a theoretical one: `XLHA::isValid` was found displacing the cursor on 2026-08-24, and its lh4 selftest had been passing only because the tag read happened to land on the probe offset by coincidence. `XArchive::getRecords` is the underlying displacer — it never saves or restores, and its reads end in `safeReadData`'s `seek(nPos + nResult)` — so any detection routine calling it at top level needs its own guard. `hasLegacyZstdMagic` was separately fixed to read the magic at offset 0 rather than `peek()` at wherever the caller left the cursor, which misrouted a long-lived device in both directions. |
